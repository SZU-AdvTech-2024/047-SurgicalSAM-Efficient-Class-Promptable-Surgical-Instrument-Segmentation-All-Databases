...
for epoch in range(num_epochs):   
    ...
    # 初始化存储图像的slot张量，形状为[7, 3, 512, 512]，这里的7代表7个前景类
    slot = torch.zeros([7, 3, 512, 512])  
    # 初始化存储有效掩码的vm张量，形状为[7, 21, 512, 512]
    vm = torch.zeros([7, 21, 512, 512])  
    # 初始化存储标签的sltarget张量
    sltarget = torch.zeros([7, 7])  
    # 创建一个包含0到6的张量，代表类别索引，且放置在GPU上
    target2 = torch.linspace(0, 6, 7, dtype = torch.long).cuda()  
    ems = torch.linspace(0, 6, 7, dtype = torch.long).cuda()  


    def shuffle_batch(x, y, z, c):
    ...
    for sam_feats, _, cls_ids, masks, class_embeddings in train_dataloader: 
        ...
        cls_ids = cls_ids.cuda(non_blocking = True)  
        masks = masks.cuda()
        class_embeddings = class_embeddings.cuda()
        # 对图像、标签和有效掩码进行随机打乱
        sam_feats, cls_ids, masks, class_embeddings = shuffle_batch(sam_feats, cls_ids, masks, class_embeddings)  
        # 获取当前批次的图像数量
        ba = len(cls_ids)  
        for i in range(ba):
            # 找到标签中值为1的索引，即对应前景类别的索引
            argl = (cls_ids[i, :].squeeze() == 1).nonzero(as_tuple = False).squeeze()  
            slot[argl] = sam_feats[i].cpu()  
            vm[argl] = masks[i].cpu()  
            sltarget[argl] = cls_ids[i].cpu()  
            ems[argl] = class_embeddings[i].cpu()

        # 将标签重新放回GPU
        cls_ids = cls_ids.cuda()  
        sam_feats = sam_feats.cuda()  
        masks = masks.cuda()  
        # 对存储的图像、类别索引和有效掩码进行随机打乱
        ...
        # 将存储的图像、类别索引和有效掩码放回GPU
        slot_c = slot.cuda()  
        target2_c = sltarget.cuda()  
        vm_c = vm.cuda()

        # 将原始图像与从存储图像库中随机采样的图像在维度0上进行拼接
        sam_feats = torch.cat([sam_feats, slot_c[first].unsqueeze(0), slot_c[second].unsqueeze(0), slot_c[third].unsqueeze(0), slot_c[four].unsqueeze(0)], dim = 0)  
        cls_ids = torch.cat([cls_ids, target2_c[first].unsqueeze(0), target2_c[second].unsqueeze(0), target2_c[third].unsqueeze(0), target2_c[four].unsqueeze(0)], dim = 0)  
        class_embeddings = torch.cat([class_embeddings, vm_c[first].unsqueeze(0), vm_c[second].unsqueeze(0), vm_c[third].unsqueeze(0), vm_c[four].unsqueeze(0)], dim = 0) 
